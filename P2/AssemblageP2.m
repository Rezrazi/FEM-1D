function [K,M,F] = AssemblageP2(app,alpha,beta,f, X, T)
...
%
   for k=1:t   % boucle sur les ?l?ments
       [ elemKi,elemMi] = mat_elem_P2(app,alpha,beta,X,T,k);  % matrice de l'?l?ment num?ro k
...
%
   for k=1:t   % boucle sur les ?l?ments
       elemFi = SM_elem_P2(app,f,X,T,k);  % SM de l'?l?ment num?ro k
